    ...
    <MotionFade>
      <section className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted">Informações da sua conta autenticada.</p>
        </div>
        ...
        <div className="grid gap-4 rounded-3xl border border-border bg-surface p-6 shadow-sm sm:grid-cols-2">
          ...
          <ProfileItem label="Criado em" value={formatDate(profile.createdAt)} />
          <ProfileItem label="Atualizado em" value={formatDate(profile.updatedAt)} />
        </div>

        <div className="space-y-4 rounded-3xl border border-border bg-surface p-6 shadow-sm">
          <div>
            <h2 className="font-primary text-2xl text-text">Histórico de palavras</h2>
            <p className="text-sm text-muted">As últimas palavras consultadas na sua conta.</p>
          </div>
          ...
          {!historyQuery.isLoading && !historyQuery.isError && historyQuery.data?.results.length === 0 ? (
            <p className="text-sm text-muted">Você ainda não pesquisou nenhuma palavra.</p>
          ) : null}

          {!historyQuery.isLoading && !historyQuery.isError && (historyQuery.data?.results.length ?? 0) > 0 ? (
            <div className="space-y-3">
